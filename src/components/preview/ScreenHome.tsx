...
export default function ScreenHome() {
  const qaItems = [
    ...
    { icon: <path d="M22 12h-4l-3 9L9 3l-3 9H2"/>, label: 'Saúde' },
  ];

  return (
    <PhoneShell label="Home · Dashboard" time="09:41">
      <div style={{ padding: '0 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {/* header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 600, color: T.text }}>Bom dia, Mari</div>
            <div style={{ fontSize: 10, color: T.fogDim, marginTop: 2 }}>sexta, 5 de junho</div>
          </div>
        ...
        <div style={{ borderRadius: 13, border: `1px solid ${T.borderHi}`, background: 'rgba(196,116,138,0.06)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', position: 'relative', overflow: 'hidden', padding: '12px 12px 12px 18px', boxShadow: `0 8px 28px rgba(0,0,0,0.14), inset 0 1px 0 ${T.shimmer}` }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 7 }}>
            ...
            <span style={{ fontSize: 8.5, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase', color: T.rose300, flex: 1 }}>Noma está online</span>
            <LivePill />
          </div>
          <div style={{ fontSize: 11, color: T.fog, lineHeight: 1.5, marginBottom: 8 }}>Pronta para resolver qualquer coisa. Do salão ao médico, da agenda ao que está pesando.</div>
        ...
        {/* Quick actions */}
        <div style={{ fontSize: 8.5, fontWeight: 600, letterSpacing: '1.1px', textTransform: 'uppercase', color: T.fogDim }}>Resolver agora</div>
        ...
        {/* Agents card */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: 8.5, fontWeight: 600, letterSpacing: '1.1px', textTransform: 'uppercase', color: T.fogDim }}>Agentes ativos</div>
          <span style={{ fontSize: 9.5, color: T.rose300 }}>Ver todos →</span>
        </div>
        <GlassCard>
          {[
            { name: 'Noma Style', desc: 'Beleza, estilo e autocuidado', icon: <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/> },
            { name: 'Noma Be', desc: 'Saúde, bem-estar e corpo', icon: <path d="M22 12h-4l-3 9L9 3l-3 9H2"/> },
            { name: 'Noma 4you', desc: 'Cotidiano, agenda e decisões', icon: <><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></> },
          ].map((ag, i) => (
