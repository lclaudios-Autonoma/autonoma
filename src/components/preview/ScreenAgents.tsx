...
export default function ScreenAgents() {
  return (
    <PhoneShell label="Agentes · Hierarquia" time="09:41">
      <div style={{ position: 'relative', zIndex: 1, padding: '0 13px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ padding: '8px 0 4px' }}>
          <div style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: 17, fontWeight: 600, color: T.text }}>Agentes</div>
          <div style={{ fontSize: 9.5, color: T.fogDim, marginTop: 1 }}>Derivados da Noma · 3 ativos</div>
        </div>
        ...
        <GlassCard style={{ padding: 12, borderColor: T.borderHi }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 6 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 9.5, color: T.rose300, fontWeight: 500 }}>Agente Principal · AutoNoma</div>
            </div>
            <LivePill />
          </div>
          <div style={{ fontSize: 10.5, color: T.fog, lineHeight: 1.5 }}>Sua companheira pessoal. Coordena todos os agentes.</div>
        </GlassCard>

        <div style={{ fontSize: 8.5, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: T.fogDim, padding: '0 2px' }}>Especializados</div>
        ...
        {agents.map((ag) => (
          <GlassCard key={ag.name} style={{ padding: 10, opacity: ag.active ? 1 : 0.42 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: ag.active ? 4 : 0 }}>
              ...
              {ag.active ? <ActivePill /> : <ActivePill label="breve" gold />}
            </div>
